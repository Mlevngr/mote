...
**[直接下载 InkNote 0.6.7 APK](https://github.com/Mlevngr/note/releases/download/v0.6.7/InkNote-0.6.7.apk)**
...
- 普通 Launcher 图标保留笔记与书写笔元素，并将整体视觉重心向右微调；左侧黄色书脊保持高对比度，亮色背景为白色，暗色背景为黑色。
...
- 选择移动或复制后，每一条 Markdown 行之间都会显示明确的蓝色插入位置，可将内容放到笔记开头、任意两行之间或文末；移动使用应用内部状态完成原子删除与插入，不会因系统剪贴板变化而降级为复制。
- 文档末尾始终保留可操作的尾部空白区，即使最后一项是图片或 PDF，也能继续向下滚动并在文末移动/粘贴；双击该区域会创建新的正文行。
